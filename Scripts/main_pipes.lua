--[[
    Tempest AI Lua Script for MAME - Pipe Version
    Author: Dave Plummer (davepl) and various AI assists
    Date: [2025-03-06] (Pipe Refactored Date)

    Overview: Refactored version using pipes instead of sockets for communication.
--]]
...
local PIPE_READ_TIMEOUT_S     = 0.5
local PIPE_RETRY_WAIT_S       = 0.01
...
-- Pipe paths (will be set by Python server)
local input_pipe_path = nil
local output_pipe_path = nil
...
-- Get pipe paths from environment or use defaults
local function get_pipe_paths()
    -- Try to get paths from environment variables set by Python
    local temp_dir = os.getenv("TEMP") or os.getenv("TMP") or "/tmp"
    input_pipe_path = temp_dir .. "/tempest_ai_input"
    output_pipe_path = temp_dir .. "/tempest_ai_output"
    
    print("Using pipe paths:")
    print("  Input:  " .. input_pipe_path)
    print("  Output: " .. output_pipe_path)
end
...
local function close_pipes()
    ...
    -- print("Pipes closed.") -- Optional: uncomment for debug
end

local function open_pipes()
    close_pipes() -- Ensure any existing pipes are closed first

    local pipe_success, err = pcall(function()
        -- Open input pipe for reading
        local input_file = emu.file("r")
        local input_result = input_file:open(input_pipe_path)
        if input_result == nil then
            input_pipe = input_file
            print("Input pipe opened: " .. input_pipe_path)
        else
            print("Failed to open input pipe: " .. tostring(input_result))
            input_file:close()
            return false
        end

        -- Open output pipe for writing
        local output_file = emu.file("w")
        local output_result = output_file:open(output_pipe_path)
        if output_result == nil then
            output_pipe = output_file
            print("Output pipe opened: " .. output_pipe_path)
            return true
        else
            print("Failed to open output pipe: " .. tostring(output_result))
            output_file:close()
            if input_pipe then
                input_pipe:close()
                input_pipe = nil
    ...
    if not pipe_success or not input_pipe or not output_pipe then
        print("Error during pipe opening: " .. tostring(err or "unknown error"))
        close_pipes() -- Ensure cleanup
        return false
    end
    return true
end
...
-- Flatten game state to binary format for sending over pipe
...
-- Send state and receive action via pipes
local function process_frame_via_pipes(rawdata)
    -- Ensure pipe connection
    if not input_pipe or not output_pipe then
    ...
    local write_success, write_err = pcall(function()
        ...
        output_pipe:write(length_header)
        output_pipe:write(rawdata)
        output_pipe:flush() -- Ensure data is written immediately
    end)

    if not write_success then
        print("Error writing to pipe: " .. tostring(write_err) .. ". Attempting reconnect.")
        close_pipes()
        open_pipes() -- Try immediate reconnect
        return 0, 0, 0, false -- Return zeros and error flag
    end
    ...
    local read_success, read_result = pcall(function()
        ...
        while elapsed < PIPE_READ_TIMEOUT_S do
            -- Try reading 3 bytes for the action (b, b, b)
            action_bytes = input_pipe:read(3)
        ...
        print("Pipe read timeout after " .. string.format("%.3f", elapsed) .. "s. Expected 3 bytes.")
        if elapsed >= PIPE_READ_TIMEOUT_S then
            print("Pipe read timeout exceeded, attempting reconnect...")
            close_pipes()
            open_pipes()
        end
    ...
    if not read_success then
        print("Error reading from pipe: " .. tostring(read_result) .. ". Attempting reconnect.")
        close_pipes()
        open_pipes()
        return 0, 0, 0, false -- Return zeros and error flag
    end
...
local function handle_ai_interaction()
    ...
    -- Default values if pipes are not connected
    local received_fire_cmd, received_zap_cmd, received_spinner_cmd = 0, 0, 0
    local pipe_ok = false
    local num_values = 0 -- Default if not sending

    -- Only attempt network ops if pipes exist and seem valid
    if input_pipe and output_pipe then
        ...
        received_fire_cmd, received_zap_cmd, received_spinner_cmd, pipe_ok = process_frame_via_pipes(frame_data)

        -- Update total bytes sent (only if pipe write was likely successful)
        if pipe_ok then -- Assuming pipe_ok implies write likely succeeded before read attempt
            total_bytes_sent = total_bytes_sent + #frame_data
        end
    else
        -- Pipes don't exist, attempt to open them periodically
        local current_time = os.time()
        if current_time - last_connection_attempt_time > CONNECTION_RETRY_INTERVAL_S then
            ...
            open_pipes() -- Attempt connection
        end
    end

    -- Store received commands (will be 0s if no pipes or read failed)
    ...
    return episode_done, pipe_ok, num_values -- Return done flag, pipe status, and num_values for display
end
...
local function frame_callback()
    ...
    local episode_done, pipe_ok, num_values_packed = handle_ai_interaction()
...
local function on_mame_exit()
    ...
    if mainCpu and mem and controls and input_pipe and output_pipe then
        ...
        print("Could not process final frame: MAME interface, controls, or pipes not available.")
    end

    close_pipes() -- Ensure pipes are closed
...
-- Get pipe paths and attempt initial connection
get_pipe_paths()
open_pipes()
...
print("Tempest AI script initialized with pipe communication and callbacks registered.")
--[[ End of main_pipes.lua ]]--
